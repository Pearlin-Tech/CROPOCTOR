...
const SettingsPage: React.FC = () => {
  ...
  return (
    <motion.div variants={pageVariants} initial="initial" animate="animate" className="min-h-screen bg-background">
      <MobileHeader title={t('settings.title', 'Settings')} onBack={() => navigate(-1)} />

      <PageLayout className="pt-4 pb-8 space-y-6">
        <h1 className="hidden lg:block text-2xl font-bold text-green-forest tracking-tight mb-2">{t('settings.title', 'Settings')}</h1>

        {/* Language & Region */}
        <div>
          <p className="text-[10px] font-bold text-brown-earth uppercase tracking-widest mb-3">{t('settings.sections.languageRegion', 'Language & Region')}</p>
          <Card padding="none" className="divide-y divide-brown-pastel/20 border-brown-pastel/30 bg-white shadow-sm">
            <button onClick={() => navigate('/settings/language')} className="w-full flex items-center gap-3 px-4 py-3.5 hover:bg-green-pastel/10 transition-colors text-left">
              ...
              <div className="flex-1">
                <p className="text-sm font-bold text-text-main">{t('profile.language', 'Language')}</p>
                <p className="text-xs text-text-secondary font-medium">{language.toUpperCase()}</p>
              </div>
            ...
            <div className="flex items-center gap-3 px-4 py-3.5">
              ...
              <div className="flex-1">
                <p className="text-sm font-bold text-text-main">{t('settings.metricUnits', 'Metric Units')}</p>
                <p className="text-xs text-text-secondary font-medium">{metricUnits ? t('settings.metric', 'Metric (acres, °C)') : t('settings.imperial', 'Imperial (hectares, °F)')}</p>
              </div>
        ...
        {/* Voice */}
        <div>
          <p className="text-[10px] font-bold text-brown-earth uppercase tracking-widest mb-3">{t('settings.sections.voiceAI', 'Voice & AI')}</p>
          <Card padding="none" className="divide-y divide-brown-pastel/20 border-brown-pastel/30 bg-white shadow-sm">
            <button onClick={() => navigate('/settings/voice')} className="w-full flex items-center gap-3 px-4 py-3.5 hover:bg-green-pastel/10 transition-colors text-left">
              ...
              <div className="flex-1">
                <p className="text-sm font-bold text-text-main">{t('profile.voiceSettings', 'Voice Settings')}</p>
                <p className="text-xs text-text-secondary font-medium">{t('settings.voiceDesc', 'Speech rate, language, output')}</p>
              </div>
              <ChevronRight className="w-4 h-4 text-brown-earth/40" />
            </button>
          </Card>
        ...
        {/* Notifications */}
        <div>
          <p className="text-[10px] font-bold text-brown-earth uppercase tracking-widest mb-3">{t('profile.notifications', 'Notifications')}</p>
          <Card padding="none" className="divide-y divide-brown-pastel/20 border-brown-pastel/30 bg-white shadow-sm">
            {[
              { key: 'weatherAlerts', label: t('settings.weatherAlerts', 'Weather Alerts'), desc: t('settings.weatherAlertsDesc', 'Rain, storms, and extreme events'), value: notifWeather, setter: setNotifWeather },
              { key: 'diseaseWarnings', label: t('settings.diseaseWarnings', 'Disease Warnings'), desc: t('settings.diseaseWarningsDesc', 'High-risk disease conditions'), value: notifDisease, setter: setNotifDisease },
              { key: 'irrigationReminders', label: t('settings.irrigationReminders', 'Irrigation Reminders'), desc: t('settings.irrigationRemindersDesc', 'Scheduled irrigation checks'), value: notifIrrigation, setter: setNotifIrrigation },
              { key: 'aiRecommendations', label: t('settings.aiRecommendations', 'AI Recommendations'), desc: t('settings.aiRecommendationsDesc', 'Daily farm intelligence updates'), value: notifAI, setter: setNotifAI },
            ].map(item => (
        ...
        {/* App Preferences */}
        <div>
          <p className="text-[10px] font-bold text-brown-earth uppercase tracking-widest mb-3">{t('settings.sections.appPreferences', 'App Preferences')}</p>
          <Card padding="none" className="divide-y divide-brown-pastel/20 border-brown-pastel/30 bg-white shadow-sm">
            <div className="flex items-center gap-3 px-4 py-3.5">
              <Moon className="w-5 h-5 text-brown-earth/70 shrink-0" strokeWidth={1.8} />
              <div className="flex-1">
                <p className="text-sm font-bold text-text-main">{t('settings.darkMode', 'Dark Mode')}</p>
                <p className="text-xs text-text-secondary font-medium">{t('settings.darkModeDesc', 'Coming in next update')}</p>
              </div>
            ...
            <div className="flex items-center gap-3 px-4 py-3.5">
              ...
              <div className="flex-1">
                <p className="text-sm font-bold text-text-main">{t('settings.offlineMode', 'Offline Mode')}</p>
                <p className="text-xs text-text-secondary font-medium">{t('settings.offlineModeDesc', 'Cache farm data for offline use')}</p>
              </div>
        ...
        {/* Privacy & Data */}
        <div>
          <p className="text-[10px] font-bold text-brown-earth uppercase tracking-widest mb-3">{t('profile.notificationsPrivacy', 'Privacy & Data')}</p>
          <Card padding="none" className="divide-y divide-brown-pastel/20 border-brown-pastel/30 bg-white shadow-sm">
            <button onClick={() => toast.info('Privacy policy opens in browser.')} className="w-full flex items-center gap-3 px-4 py-3.5 hover:bg-green-pastel/10 transition-colors text-left">
              ...
              <span className="flex-1 text-sm font-bold text-text-main">{t('profile.privacy', 'Privacy Policy')}</span>
              <ChevronRight className="w-4 h-4 text-brown-earth/40" />
            </button>
            <button onClick={() => toast.success('Farm data downloaded.')} className="w-full flex items-center gap-3 px-4 py-3.5 hover:bg-green-pastel/10 transition-colors text-left">
              ...
              <div className="flex-1">
                <p className="text-sm font-bold text-text-main">{t('settings.exportData', 'Export My Data')}</p>
                <p className="text-xs text-text-secondary font-medium">{t('settings.exportDataDesc', 'Download your farm history')}</p>
              </div>
              <ChevronRight className="w-4 h-4 text-brown-earth/40" />
            </button>
            <button onClick={() => toast.warning('This will delete all local data.')} className="w-full flex items-center gap-3 px-4 py-3.5 hover:bg-red-50 transition-colors text-left">
              ...
              <div className="flex-1">
                <p className="text-sm font-bold text-muted-danger">{t('settings.clearData', 'Clear Local Data')}</p>
                <p className="text-xs text-brown-earth/60 font-medium">{t('settings.clearDataDesc', 'Remove all cached farm data')}</p>
              </div>
              <ChevronRight className="w-4 h-4 text-brown-earth/40" />
            </button>
          </Card>
        ...
        <button onClick={() => navigate('/about')} className="w-full flex items-center gap-3 px-4 py-3.5 bg-white rounded-2xl border border-brown-pastel/30 hover:bg-green-pastel/10 transition-colors shadow-sm text-left">
          ...
          <div className="flex-1 text-left">
            <p className="text-sm font-bold text-text-main">{t('profile.about', 'About Cropoctor')}</p>
            <p className="text-xs text-text-secondary font-medium">{t('profile.version', 'Version')} 1.0.0 · {t('app.platform', 'Agricultural Intelligence Platform')}</p>
          </div>
          <ChevronRight className="w-4 h-4 text-brown-earth/40" />
        </button>
