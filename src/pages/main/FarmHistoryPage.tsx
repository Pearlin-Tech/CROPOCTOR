            ...
            <motion.div key={item.id} variants={cardVariants}>
              <Card padding="md" className="flex items-start gap-3 hover:shadow-card-lg transition-shadow cursor-pointer bg-white border-brown-pastel/30 shadow-sm hover:border-green-pastel/50" onClick={() => {}}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-text-secondary font-medium mt-0.5 line-clamp-2">{item.summary}</p>
                </div>
                <span className="text-xs text-brown-earth/60 font-medium shrink-0 mt-0.5">{timeAgo(item.timestamp)}</span>
              </Card>
            </motion.div>
          ))}
        </motion.div>
